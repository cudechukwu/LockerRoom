...
-- Step 1: Add scope and visibility columns to playbooks table
...
-- Step 2: Create playbook shares table for granular sharing control
...
-- Step 3: Create play shares table for individual play sharing
...
-- Step 5: Enable Row Level Security (RLS) Policies for new tables
ALTER TABLE playbook_shares ENABLE ROW LEVEL SECURITY;
ALTER TABLE play_shares ENABLE ROW LEVEL SECURITY;

-- Step 6: Create RLS policies for playbook_shares
CREATE POLICY "Users can view their own shares" ON playbook_shares
  FOR SELECT USING (playbook_shares.shared_by = auth.uid());

CREATE POLICY "Public can view public playbook shares" ON playbook_shares
  FOR SELECT USING (playbook_shares.share_scope = 'view' AND playbook_shares.shared_with IS NULL);
...
CREATE POLICY "Users can update their own shares" ON playbook_shares
  FOR UPDATE USING (playbook_shares.shared_by = auth.uid());

CREATE POLICY "Users can delete their own shares" ON playbook_shares
  FOR DELETE USING (playbook_shares.shared_by = auth.uid());

-- Step 7: Create RLS policies for play_shares
CREATE POLICY "Users can view their own play shares" ON play_shares
  FOR SELECT USING (play_shares.shared_by = auth.uid());

CREATE POLICY "Public can view public play shares" ON play_shares
  FOR SELECT USING (play_shares.share_scope = 'view' AND play_shares.shared_with IS NULL);
...
CREATE POLICY "Users can update their own play shares" ON play_shares
  FOR UPDATE USING (play_shares.shared_by = auth.uid());

CREATE POLICY "Users can delete their own play shares" ON play_shares
  FOR DELETE USING (play_shares.shared_by = auth.uid());

-- Step 8: Update existing playbooks policies to handle scope and visibility
...
-- Step 9: Create default team playbooks for new teams
