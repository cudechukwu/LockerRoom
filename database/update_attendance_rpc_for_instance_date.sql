-- Update insert_event_attendance_manual RPC to support instance_date for recurring events
-- This allows coaches to mark attendance for specific instances of recurring events
...
CREATE OR REPLACE FUNCTION insert_event_attendance_manual(
  ...
  p_instance_date DATE DEFAULT NULL  -- 🔥 NEW: For recurring events, specify the instance date
)
...
BEGIN
  ...
  -- 🔥 FIX: Include instance_date in the WHERE clause for recurring events
  IF EXISTS (
    SELECT 1 FROM event_attendance
    WHERE event_id = p_event_id
    AND user_id = p_user_id
    AND (instance_date IS NOT DISTINCT FROM p_instance_date)  -- NULL-safe comparison
  ...
  ELSE
    ...
    INSERT INTO event_attendance (
      ...
      user_id,
      team_id,
      instance_date,  -- 🔥 NEW: Include instance_date
      check_in_method,
      checked_in_at,
      status,
      is_late,
      ...
      p_user_id,
      p_team_id,
      p_instance_date,  -- 🔥 NEW: Include instance_date
...
GRANT EXECUTE ON FUNCTION insert_event_attendance_manual(
  UUID, UUID, UUID, VARCHAR, VARCHAR, TIMESTAMPTZ, BOOLEAN, INTEGER, VARCHAR, DATE
) TO authenticated;
