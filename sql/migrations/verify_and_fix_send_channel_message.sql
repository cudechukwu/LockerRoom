-- Verify and fix send_channel_message function
-- Run this if the function update didn't work

-- First, drop the function completely
drop function if exists public.send_channel_message(uuid, text, text, uuid);

-- Now recreate it with the fix
...
begin
  ...
  -- Only fetch parent if parent_message_id was provided
  if p_parent_message_id is not null then
    select id,
           channel_id,
           team_id,
  ...
  return jsonb_build_object(
    ...
    'parent', case
      when v_parent_fetched then jsonb_build_object(
        'id', v_parent.id,
